...
namespace G.Util
{
	...
	{
		private Queue<byte[]> q16384 = new Queue<byte[]>();
		...
		public byte[] CheckOut16384()
		{
			lock (q16384) { return CheckOut(q16384, 16384); }
		}
		...
		public byte[] CheckOut(int size)
		{
			...
			if (size > 8192) return CheckOut16384();
			if (size > 4096) return CheckOut8192();
			if (size > 2048) return CheckOut4096();
			if (size > 1024) return CheckOut2048();
			if (size > 512) return CheckOut1024();
			if (size > 256) return CheckOut512();
			if (size > 128) return CheckOut256();
			if (size > 64) return CheckOut128();
			if (size > 32) return CheckOut64();
			if (size > 16) return CheckOut32();
			return CheckOut16();
		}
	}
}
